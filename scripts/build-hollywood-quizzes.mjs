...
/**
 * 이 점수보다 낮은 배우는 힌트로 쓰지 않는다. 0 이면 걸러내지 않는다.
 *
 * 점수는 '배역 순서 3위 안에 든 작품들의 IMDb 평점 투표 수 합' 이다(build-imdb-fame.mjs).
 * 기준값별로 첫 힌트에 나오는 배우의 인지도를 재서 골랐다.
 *
 *        쉬움편수  어려움편수   첫 힌트 인지도 중앙값(쉬움/어려움)  주연 경험 0인 배우
 *   없음     401      399           64만 / 20만                 13% / 26%
 *   1점      401      365           70만 / 46만                  0% /  0%
 *   20만     401      259          100만 / 102만                 0% /  0%
 *   60만     361      168          174만 / 182만                 0% /  0%
 *
 * 20만으로 정했다. 쉬움은 401편 그대로인데 어려움의 첫 힌트 인지도가 두 배 넘게
 * 오른다. 60만까지 올리면 쉬움까지 깎이기 시작한다.
 *
 * 어려움이 259편으로 줄지만 5판씩 돌려도 같은 영화가 금방 되돌아오지 않는다.
 * 출연진이 얕은 영화(나는 전설이다는 아는 배우가 셋뿐이다)는 어려움에서 빠지는데,
 * 원래 그런 영화에 어려움을 내면 안 되는 것이 맞다.
 */
const minFame = Number(flags["min-fame"] ?? 200_000)
